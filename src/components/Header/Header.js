...
import Logo1 from "./Headercomp/Logo1";
...
const Header = (finishedLoading,sectionsRef ) => {
  ...
  return (
    <>
      ...
      <motion.div
        ...
        transition={{ opacity: { delay: finishedLoading ? 0 : 4.4, duration: 0 } }}
        className={`w-full fixed ${ShowElement ? `bg-opacity-70 shadow-xl` : `bg-opacity-0 `} bg-primary flex 
      justify-between px-6 sm:px-12 py-2 sm:py-4  transition duration-4000 translate-y-0 z-20`}
      >
        
        <a href="#">
          <Logo1 className="w-auto h-8" />
        </a>
